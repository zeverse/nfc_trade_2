...
async function main() {
    ...
    const signer_wallet = new Wallet(
        "b6d51d89f7c9bdeafebc8ad6b3c6cd208895a1947997024c4a4950e8845dcd8a",
    );
    const signer = await signer_wallet.connect(provider);

    const POS = await ethers.getContractFactory("POS");
    const Vault = await ethers.getContractFactory("VaultContract");
    const vault = await Vault.attach(
        "0x0731fE55F95B5b986F4EBd187315fB1F4C823b94",
    );
    ...
    const paymentData = await Vault.interface.encodeFunctionData("pay", [
        data.sessionId,
        data.pos,
        data.sender,
        data.token,
...
async function signatureReconfimation(
    ...
    // To sign the 32 bytes of data, make sure you pass in the data
    return await user.signMessage(messageHashBinary);
}
